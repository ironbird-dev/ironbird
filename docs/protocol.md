...
A successful operation returns `{ "ok": true, "result": { ... } }`. A failed operation returns `{ "ok": false, "error": { "code": "...", "message": "...", "details": { ... } } }`. Both use HTTP 200. Other statuses are reserved for transport problems: 401 for a missing or wrong token, 404 for unknown routes, and 500 for daemon faults.
