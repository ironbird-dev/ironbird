...
| The same run on an Android emulator is recorded, and an Android target is set from the result | met | `pnpm measure -- --target android`: reduced motion stale 0/300, p95 1328 ms; full motion stale 54/300, p95 1462 ms. Android target: stale ≤ 1% and p95 < 1.5 s, based on the reduced-motion run, which is artifact-free; the full-motion 18% figure is a measurement floor from the Expo Go dev-menu overlay left on screen for the session (see Q5 finding), not a reliable full-motion baseline |
...
| `verify-bundle` passes on a production `expo export` and fails on a deliberately broken build | met | `expo export --platform ios` then `verify-bundle dist`: exit 0, 4 files scanned, `found: []`. `EXPO_PUBLIC_FORCE_BRIDGE=1 expo export` then `verify-bundle dist-forced`: exit 1, marker found in `dist-forced/_expo/static/js/ios/index-bbc76142eb9a98d2c66b84414d85207a.hbc` at offset 169655 |
...
The iOS p95 miss is structural, not a variance artifact: `payment.start` is 1 of the 5 cycle positions (20% of steps, comfortably past p80), and every `payment.start` sample carries the payment fake's fixed 800 ms deliberate timer (`latencyMs` 300 + `echoDelayMs` 500 in `examples/checkout/src/ironbird/fakes/api.ts`) plus roughly 545 ms of `xcrun simctl io screenshot` capture time, landing at p50 1611 ms (full) / 1649 ms (reduced) and p95 1657 ms / 1672 ms; non-`payment.start` steps sit at p95 ~741 ms in both arms. This does not change with motion, the app, the fake, or the cycle, so the roadmap's "narrow if needed" clause (iOS stale rate above 5%) does not apply here — stale rate is not the problem. The p95 verdict is the maintainer's gate decision to make.
...
- `expo export`'s Metro transform cache does not key on the `EXPO_PUBLIC_FORCE_BRIDGE` environment variable: running the plain and forced exports back to back without clearing the cache can silently reuse a stale transform of `index.js` from an earlier invocation, making a "plain" export wrongly include the bridge (if a forced export ran first) or a "forced" export wrongly exclude it (if a plain export ran first). Pass `--clear` on both `expo export` invocations when proving `verify-bundle` to get a trustworthy result; this is what Step 1's recorded runs did.
- iOS per-command latency is cleanly bimodal (non-`payment.start` p95 741 ms, `payment.start` p95 ~1660 ms) with zero overlap, which made the p95 miss easy to attribute; a harness that reported per-command percentiles alongside the pooled ones would make this diagnosis immediate without needing to read the raw `steps-*.jsonl` files.
